    ...
    <>
      ...
      <section className="py-20 bg-nv-navy">
        <div className="section-container">
          <FadeIn><p className="section-tag mb-8">Featured</p></FadeIn>
                ...
                <FadeIn key={post.slug} delay={i * 0.1}>
                  <Link href={`/blog/${post.slug}`} className="group block card-base card-hover h-full relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-2 mb-4 flex-wrap">
                      ...
                      {post.division && (
                        <span className="text-[10px] text-nv-muted font-display font-semibold uppercase tracking-wider">
                          {divisions.find((d) => d.slug === post.division)?.name}
                        </span>
                      )}
                    </div>
                    <h2 className="font-display font-extrabold text-xl text-nv-txt mb-3 group-hover:text-white transition-colors leading-snug">{post.title}</h2>
          ...
          <FadeIn><p className="section-tag mb-6">All Posts</p></FadeIn>
                  ...
                  <Link href={`/blog/${post.slug}`} className="group flex items-start gap-5 card-base card-hover">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        ...
                        <span className="text-[10px] text-nv-muted">{post.date} · {post.readTime}</span>
                      </div>
